...
Note: Some pretrained model and image-quality-assessment weights are stored using Git LFS. Please install Git LFS before cloning the repository if you want to obtain these files correctly.
...
LR_path specifies the directory containing the low-resolution input images.

SR_path specifies the directory used to save the reconstructed super-resolution images.
...
To evaluate all supported metrics, including LPIPS, DISTS, NIQE, MUSIQ, CLIP-IQA, and FID, run:
...
The evaluation includes LPIPS, DISTS, NIQE, MUSIQ, CLIP-IQA, and FID. The required pretrained metric weights are provided in the weights/ directory.

If CLIP-IQA or FID needs to be disabled separately, use --skip_clipiqa or --skip_fid, respectively.

When reproducing the experiments on another machine, replace the absolute paths above with the corresponding local paths.
